...
This project analyzes **United Nations General Assembly voting behavior** over time to understand global power dynamics, ideological polarization, and alignment among major world powers. Instead of relying on static figures, the analysis is presented primarily through **interactive HTML visualizations**, enabling clearer interpretation and more effective presentation.

The project culminates in a **web-based narrative dashboard**, with `index.html` serving as the main entry point for exploring all results.
...
```
/ (project root)
├── data/                      # UN voting datasets
│   ├── AgreementScores.csv
│   ├── AgreementScoresAll_Jun2024.csv
│   └── IdealpointestimatesAll_Jun2024.csv
├── plots/                     # Interactive HTML visualizations (Plotly / Bokeh)
...
├── index.html                 # Main interactive voting blog and narrative dashboard
...
The dataset contains country-level voting decisions (`Yes`, `No`, `Abstain`) and enables long-run analysis of ideological alignment, polarization, and geopolitical trends within the UN.
...
- Construction of voting alignment and polarization metrics
...
- UN voting behavior exhibits clear historical phases (Cold War, post–Cold War, and multipolar era).
- Major power alignments are dynamic rather than fixed.
- Interactive visualizations reveal patterns not evident in raw data or static plots.
...
3. Explore the interactive visualizations inside the `plots/` folder  

> **Note:** No Python execution is required to view the results.
